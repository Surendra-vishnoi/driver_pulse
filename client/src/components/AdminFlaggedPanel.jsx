...
function AdminFlaggedPanel() {
  ...
  const filtered = flagged.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.trip_id.toLowerCase().includes(q) && !f.driver_id.toLowerCase().includes(q)) return false
    }
    return true
  })
  ...
  useEffect(() => { setPage(0) }, [filterSeverity, filterType, searchQuery])
    ...
    return (
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Running flagging engine on all sensor data</p>
        </div>
      </section>
    )
  }
...
export default AdminFlaggedPanel
