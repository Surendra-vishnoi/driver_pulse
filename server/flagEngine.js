...
const ACCEL_CSV = path.join(DATA_DIR, "sensor_data", "accelerometer_data.csv");
const AUDIO_CSV = path.join(DATA_DIR, "sensor_data", "audio_intensity_data.csv");
const TRIPS_CSV = path.join(DATA_DIR, "trips", "trips.csv");
...
function loadData() {
  const accelRows = loadCsv(ACCEL_CSV).map((r) => ({
  ...
  const audioRows = loadCsv(AUDIO_CSV).map((r) => ({
    audio_id: r.audio_id,
    trip_id: r.trip_id,
    timestamp: r.timestamp,
    ts: toTimestamp(r),
  ...
  const tripRows = loadCsv(TRIPS_CSV);
  const tripDriverMap = {};
  for (const t of tripRows) {
    tripDriverMap[t.trip_id] = t.driver_id;
  }

  return { accelRows, audioRows, tripDriverMap };
}
...
function motionScore(lateralXy) {
  if (lateralXy >= 5.0) return clip(0.85 + (lateralXy - 5.0) / (8.68 - 5.0) * 0.15, 0.85, 1.0);
  if (lateralXy >= 3.5) return clip(0.65 + (lateralXy - 3.5) / (5.0 - 3.5) * 0.20, 0.65, 0.85);
  if (lateralXy >= 2.0) return clip(0.40 + (lateralXy - 2.0) / (3.5 - 2.0) * 0.25, 0.40, 0.65);
  return clip(lateralXy / 2.0 * 0.40, 0.0, 0.40);
}
...
function severity(combined) {
  if (combined >= 0.80) return "high";
...
function detectSustainedStress(flaggedRows) {
  ...
  for (const [tripId, rows] of Object.entries(byTrip)) {
    const moderates = rows.filter(
      (r) =>
        r.flag_type === "moderate_brake" ||
        r.flag_type === "audio_elevated"
    );
    ...
    for (let i = 0; i < moderates.length; i++) {
      ...
      for (let j = i + 1; j < moderates.length; j++) {
        ...
        if (gap <= SUSTAINED_STRESS_WINDOW_SEC) {
          cluster.push(j);
        }
      }
...
function runFlaggingPipeline() {
  const { accelRows, audioRows, tripDriverMap } = loadData();
...
module.exports = { runFlaggingPipeline, computeStats };
